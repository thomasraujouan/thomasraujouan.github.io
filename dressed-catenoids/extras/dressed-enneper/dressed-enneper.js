...
const objPath = "./dressed-enneper.obj";
...
function init() {
  ...
  const distance = 20; // cf fittingDistance()
  const position = {
    x: 1*Math.sqrt((2 * (distance * distance)) / 5),
    y: 1*Math.sqrt((2 * (distance * distance)) / 5),
    z: 1*Math.sqrt((2 * (distance * distance)) / 5),
  };
